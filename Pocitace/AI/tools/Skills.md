- https://skills.sh/ - a big skill library
	- https://skills.sh/steveyegge/beads - beads
	- https://skills.sh/obra/superpowers - these are reeeally good
	- https://skills.sh/obra/superpowers-skills/getting-started-with-skills
	- https://skills.sh/obra/episodic-memory/remembering-conversations
- https://github.com/wshobson/agents - Intelligent automation and multi-agent orchestration for Claude Code
### Currently used skills
```sh
> npx skills ls -h
beads ~/.agents/skills/beads
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
brainstorming ~/.agents/skills/brainstorming
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
dispatching-parallel-agents ~/.agents/skills/dispatching-parallel-agents
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
executing-plans ~/.agents/skills/executing-plans
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
find-skills ~/.agents/skills/find-skills
  Agents: Codex, OpenCode, Replit
ralph-tui-create-beads ~/.agents/skills/ralph-tui-create-beads
  Agents: Codex, OpenCode, Replit
ralph-tui-prd ~/.agents/skills/ralph-tui-prd
  Agents: Codex, OpenCode, Replit
receiving-code-review ~/.agents/skills/receiving-code-review
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
requesting-code-review ~/.agents/skills/requesting-code-review
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
subagent-driven-development ~/.agents/skills/subagent-driven-development
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
systematic-debugging ~/.agents/skills/systematic-debugging
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
test-driven-development ~/.agents/skills/test-driven-development
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
verification-before-completion ~/.agents/skills/verification-before-completion
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 more
writing-plans ~/.agents/skills/writing-plans
  Agents: Augment, Claude Code, Codex, Kilo Code, OpenCode +1 mor
```
- https://github.com/mattpocock/skills - Matt Pocock’s practical engineering agent skills, also published on skills.sh
- https://github.com/openclaw/agent-skills/ - canonical shared skills repo for OpenClaw/coding-agent workflows such as review closeout and validation
	- https://github.com/openclaw/agent-skills/blob/main/skills%2Fautoreview%2FSKILL.md - pre-commit/ship review skill: structured, security-aware code review with Codex by default, plus optional Claude or Pi engines.
- https://github.com/virgiliojr94/book-to-skill - tool for turning technical books, docs, or source collections into reusable agent skills
- https://github.com/almendili/skills/ - portable, harness-agnostic agent skill packs; currently includes an architecture-map skill that visualizes repository structure and code paths (last update 08/2026)
- https://github.com/addyosmani/agent-skills - Addy Osmani's production-grade engineering skills for AI coding agents; 24 skills covering the full dev lifecycle (spec → plan → build → test → review → ship) (last update 07/2026)
- https://github.com/Sahir619/fable-method (last update 07/2026) - FABLE method for turning Claude skills into reusable, executable agent workflows; repo referenced from Claude Fable 5 / loop-engineering material.
