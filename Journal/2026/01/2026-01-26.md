...
- there are some registered `CustomNotification`s
	...
	- they also have a name, a description and the `EventType` they can handle
...
- there will be an `EventSubscriber` implementation which will
	...
	- for each of the matching subscriptions:
		- use its `templateParamsFor(event)` method to get template params
		- call `NotificationService.sendCustomNotification()`
