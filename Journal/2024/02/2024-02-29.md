...
```bash
docker run --rm --name some-postgres -p 5432:5432 -e POSTGRES_USER=test -e POSTGRES_PASSWORD=Passw0rd -e POSTGRES_DB=test -d postgres
```
## MS SQL in docker
...
docker logs -f oracle21 # wait for 
```
